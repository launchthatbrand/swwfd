...
import { upsertMondayTouchRecordImpl } from "./mondayTouchesImpl";
...
const getRoutingConfig = () => {
  const issues: string[] = [];
  const apiKey = normalizeValue(process.env.MONDAY_API_KEY);
  const contactBoardId = normalizeValue(process.env.MONDAY_BOARD_ID);
  const countyBoardId = normalizeValue(process.env.MONDAY_ROUTING_COUNTY_BOARD_ID);
  const districtBoardId = normalizeValue(process.env.MONDAY_ROUTING_DISTRICT_BOARD_ID);
  ...
  return {
    ...
    config: {
      ...
      countyBoardId,
      districtBoardId,
      countyNameColumnId: normalizeColumnId(
        process.env.MONDAY_ROUTING_COUNTY_NAME_COLUMN_ID,
      ),
      countyDistrictColumnId: normalizeColumnId(
        process.env.MONDAY_ROUTING_COUNTY_DISTRICT_COLUMN_ID,
      ),
      countyActiveColumnId: normalizeColumnId(
        process.env.MONDAY_ROUTING_COUNTY_ACTIVE_COLUMN_ID,
      ),
      districtCodeColumnId: normalizeColumnId(
        process.env.MONDAY_ROUTING_DISTRICT_CODE_COLUMN_ID,
      ),
      districtOwnerColumnId: normalizeColumnId(
        process.env.MONDAY_ROUTING_DISTRICT_OWNER_COLUMN_ID,
      ),
      districtActiveColumnId: normalizeColumnId(
        process.env.MONDAY_ROUTING_DISTRICT_ACTIVE_COLUMN_ID,
      ),
...
// Uses shared callMondayGraphQL from mondayGraphQL.ts (MONDAY_API_KEY from env).
...
const getRoutingBoardItems = async (boardId: string) => {
  interface Data {
    boards?: {
      items_page?: {
        items?: {
...
const getCountyDistrictMappings = async (config: MondayRoutingConfig) => {
  ...
  const rows = await getRoutingBoardItems(config.countyBoardId);
...
const getDistrictOwnerMappings = async (config: MondayRoutingConfig) => {
  ...
  const rows = await getRoutingBoardItems(config.districtBoardId);
  const mappings = new Map<string, string>();
  for (const row of rows) {
    const columns = row.column_values ?? [];
    const activeColumn =
...
const updateContactOwnerAndDistrict = async (args: {
  boardId: string;
  itemId: string;
  ownerId: string;
  ...
  interface BoardColumnsData {
    boards?: Array<{
      columns?: Array<{ id?: string | null; type?: string | null }>;
    }>;
  }
  const boardData = await callMondayGraphQL<BoardColumnsData>(
    `query ResolveContactBoardColumns($boardId: ID!) {
  ...
  const peopleColumnId =
    ...
    )?.id ?? CONTACT_BOARD_OWNER_COLUMN_FALLBACK;

  const ownerAsNumber = Number(args.ownerId);
  const columnValues: Record<string, unknown> = {
    [ROUTING_DISTRICT_COLUMN_ID]: { label: args.districtLabel },
    [peopleColumnId]: {
      personsAndTeams: [
        {
          id: Number.isFinite(ownerAsNumber) ? ownerAsNumber : args.ownerId,
          kind: "person",
        },
      ],
    },
  ...
  await callMondayGraphQL<{ change_multiple_column_values?: { id?: string } }>(
    `mutation UpdateContactOwnerAndDistrict($boardId: ID!, $itemId: ID!, $columnValues: JSON!) {
...
export const getMondayDistrictRoutingStatus = async (): Promise<MondayDistrictRoutingStatus> => {
  ...
  if (!configResult.ok) {
    return {
      ok: false,
      enabled: false,
      contactBoardId: normalizeValue(process.env.MONDAY_BOARD_ID) || null,
      countyBoardId: normalizeValue(process.env.MONDAY_ROUTING_COUNTY_BOARD_ID) || null,
      districtBoardId: normalizeValue(process.env.MONDAY_ROUTING_DISTRICT_BOARD_ID) || null,
      countyMappingsCount: 0,
      districtOwnerMappingsCount: 0,
      contactBoardUrl: buildBoardUrl(normalizeValue(process.env.MONDAY_BOARD_ID) || null),
      countyBoardUrl: buildBoardUrl(
        normalizeValue(process.env.MONDAY_ROUTING_COUNTY_BOARD_ID) || null,
      ),
      districtBoardUrl: buildBoardUrl(
        normalizeValue(process.env.MONDAY_ROUTING_DISTRICT_BOARD_ID) || null,
      ),
...
export const assignMondayContactOwnerByDistrict = async (args: {
  ...
  const { config } = configResult;
  try {
    const contactItem = await getContactItemForRouting(itemId);
    if (!contactItem) {
      return createRoutingResult({
        ok: false,
        status: "error",
    ...
    await updateContactOwnerAndDistrict({
    ...
    try {
      await upsertMondayTouchRecordImpl({
