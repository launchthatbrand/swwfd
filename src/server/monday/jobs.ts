...
const readColumnText = (item: MondayJobItem, columnId: string) => {
  const value = item.column_values?.find((column) => column.id === columnId)?.text?.trim();
...
const readLinkColumnUrl = (item: MondayJobItem, columnId: string) => {
  ...
  if (column.value) {
    try {
      ...
      // Ignore parse errors and fall back to text value.
    }
  }
...
export const listMondayJobs = async (args?: {
  boardId?: string;
  limit?: number;
  search?: string;
  district?: string;
  ...
  const boardId =
    args?.boardId?.trim() ||
    env.MONDAY_JOBS_BOARD_ID?.trim() ||
    DEFAULT_MONDAY_JOBS_BOARD_ID;
  if (!boardId) {
    throw new Error("MONDAY_JOBS_BOARD_ID is missing");
  }

  const limit = Math.min(Math.max(args?.limit ?? 200, 1), 500);
  const search = args?.search?.trim().toLowerCase() ?? "";
  const districtFilter = args?.district?.trim().toLowerCase() ?? "";
  const onlyAvailable = args?.onlyAvailable ?? true;
  ...
  const jobs = (board?.items_page?.items ?? [])
    .map((item): MondayJobListing | null => {
      ...
      const locationSecondary = readColumnText(item, JOB_LOCATION_SECONDARY_COLUMN_ID);
      ...
      const contractorPrimary = readColumnText(item, JOB_CONTRACTOR_PRIMARY_COLUMN_ID);
      const contractorSecondary = readColumnText(item, JOB_CONTRACTOR_SECONDARY_COLUMN_ID);
    ...
    .filter((job): job is MondayJobListing => job !== null)
    .filter((job) => {
      if (onlyAvailable && !job.isAvailable) return false;
      if (districtFilter.length > 0) {
        const haystack = `${job.district ?? ""} ${job.location ?? ""} ${job.locationSecondary ?? ""}`.toLowerCase();
